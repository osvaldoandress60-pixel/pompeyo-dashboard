...
# Generar JSON minificado
datos_json_str = json.dumps(datos, ensure_ascii=False, separators=(',', ':'))
    ...
    <div class="container">
        ...
        <div class="table-container">
            <table id="tabla-vehiculos">
                <thead>
                    <tr>
                        <th>Accion</th><th>Patente</th><th>Marca</th><th>Gerencia</th><th>Modelo</th><th>Categoria</th><th>Fecha Entrega</th><th>Llave</th><th>FED</th><th>Genesis</th><th>Sol Precio</th><th>Ent Precio</th><th>Sol Factura</th><th>Factura</th><th>SLA1</th><th>SLA2</th><th>SLA3</th>
                    </tr>
                </thead>
...
# Escribir datos
html_mid = f'''let datosOriginales = {datos_json_str};
let datosFiltrados = [];
...
function mostrarTabla() {{
    ...
    datosFiltrados.forEach(v => {{
        ...
        tr.innerHTML = `<td><button class="btn-primary" onclick="abrirModalEditar('${{v.Patente}}')" style="padding: 5px 10px; font-size: 0.8em;">Editar</button></td>
            ...
            <td>${{v.Categoria}}</td><td>${{v["Fecha Entrega a CPD"]}}</td><td>${{v["Copia llave"] ? "OK" : ""}}</td>
            <td>${{v.FED ? "OK" : ""}}</td><td>${{v["Confirmación Genessis"] || "N/A"}}</td>
            <td>${{v["Fecha Solicitud Precio"] || "-"}}</td><td>${{v["Fecha Entrega Precio"] || "-"}}</td>
            <td>${{v["Fecha Solicitud Factura CND"] || "-"}}</td><td>${{v["Fecha Factura CND"] || "-"}}</td>
            <td><strong>${{s1 > 0 ? s1 : "-"}}</strong></td><td><strong>${{s2 > 0 ? s2 : "-"}}</strong></td><td><strong>${{s3 > 0 ? s3 : "-"}}</strong></td>`;
        tb.appendChild(tr);
    }});
}}
...
# Combine and save
html_final = html_start + html_mid

with open('index.html', 'w', encoding='utf-8') as f:
    f.write(html_final)

print(f"HTML generado exitosamente - {len(datos)} registros embebidos")
